...
void removeTail(List *&L)
{
    if(L->p_tail!= nullptr )
    {
        if(L->p_tail == L->p_head)// nếu chỉ có 1 phần tử 
        {
            delete L->p_tail;
            L->p_head = nullptr;
            L->p_tail = nullptr;
...
void removeBefore(List *&L, int val)
{
    ...
    while (current->p_next != nullptr && current->p_next->key != val)
    {
        prev = current;
        current = current->p_next;
    }

    if (current->p_next != nullptr && current->p_next->key == val)
    {
        if (prev == nullptr)
        {
